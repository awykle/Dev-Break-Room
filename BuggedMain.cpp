...

{
	...
	do {
		...
		while (!stay) {
			...
			cout << "[H]it or [S]tay? ([E]xit)\n\n"; // 'H' or 'S' to play. 'E' will return to main menu
			cin >> choice;
	...
	} while (endBlackJack == false); //ends game and returns to menu if true
...
void turn(int& playerTotal, char choice, bool& stay) {
	...
	if (choice == 'H' || choice == 'h') {
		...
		if ((playerTotal += getCard()) >= 21) {
			...
			stay = true;
			endBlackJack = false ; //continue playing until user exits game
		...
		endBlackJack = true; // end do while loop to return to main menu
	}
		...
		cout << "ERROR: Enter 'H' to receive another card, 'S' to stay, " 
			<< "or 'E' to return to main menu.";
		endBlackJack = false; //continue playing until user exits game
	}
