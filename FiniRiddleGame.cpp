...
#include <string>
#include <iomanip>
#include <stdlib.h> //library included for use in randNum function

using namespace std;

int randNum(int, int);
void answerCheck(string, string);
int plyrChance = 5, plyrScore = 0, plyrDifficulty, quesNum, correctAnswers = 3; //Global variables to be used across functions

int main()
{
	plyrDifficulty = correctAnswers / 3; //3 correct players answers modifies questions numbers that be generated via RandNum
	string corrAnswer, userAnswer;
	string errorMess = "Sorry there has been a problem with the program. Please close and try again."; //error message
	...
	cout << "Each answer is only one word beginning with a capital letter.\n"; //indicating format userAnswer needs to be in
	cout << "You can only be wrong 5 times so take your time and think things over!\n\n\n";


	while (plyrChance > 0 || correctAnswers <= 15) //switch statement that will loop until player chance is equal to zero
	{

		quesNum = randNum(plyrDifficulty, 10); //function returns a value which is then assigned to a variable to be used in following switch statement
		switch (quesNum)
		{
		case 1:// List of 50 riddles.
...
void answerCheck(string n, string m)// Checks for incorrect or correct answer and modifies variable values depending on the outcome.
{
	if (n == m) // 
	{
		cout << "\n\nThat's correct!" << setfill('*') << setw(10) << "\n";
		cout << "You still have " << plyrChance << " chances left!" << setfill('*') << setw(10);
		plyrScore = plyrScore + (100 * plyrDifficulty); //scales player difficulty scale based upon difficulty level
		cout << "\tYour current score: " << plyrScore << "!\n\n";
		correctAnswers++; // records a correct answer
	}
	else
	{
		--plyrChance;
